    ...
    <div className="min-h-screen bg-gray-100 p-4 md:p-8">
      ...
      <div className="max-w-md mx-auto bg-white p-6 rounded-xl shadow-lg mb-10 border border-gray-200 no-print">
        <h1 className="text-2xl font-bold mb-4 text-gray-800 text-center">
          {editingId ? '📝 Editing Badge' : '🏷️ Badge Creator'}
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2">
            <button type="submit" className={`flex-grow py-3 rounded-lg font-bold text-white shadow-md transition-all active:scale-95 ${editingId ? 'bg-orange-500 hover:bg-orange-600' : 'bg-blue-600 hover:bg-blue-700'}`}>
            ...
            {editingId && (
              <button 
                ...
                className="px-4 py-3 bg-gray-200 rounded-lg font-bold text-gray-600 hover:bg-gray-300"
              >
                Cancel
              </button>
            )}
        ...
        {staff.length > 0 && (
          ...
          >
            🖨️ Print {staff.length} Badge{staff.length > 1 ? 's' : ''}
          </button>
        )}
      </div>
        ...
        >
          {staff.map((person) => (
            <div key={person.id} className="relative group flex justify-center">
              ...
              {/* Management Buttons - Hidden on Print */}
              <div className="absolute -top-3 -right-2 flex gap-2 no-print">
                 <button 
                  onClick={() => startEdit(person)} 
                  className="bg-orange-500 hover:bg-orange-600 text-white w-8 h-8 rounded-full shadow-md flex items-center justify-center font-bold"
                 >
                  ✎
                 </button>
                 <button 
                  onClick={() => deleteStaff(person.id)} 
                  className="bg-red-600 hover:bg-red-700 text-white w-8 h-8 rounded-full shadow-md flex items-center justify-center font-bold"
                 >
                  ✕
                 </button>
              </div>
